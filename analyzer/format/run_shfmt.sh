...
readonly FILE_CONF=".editorconfig"
readonly FILE_LOG="${PATH_SCRIPTDIR}""/shfmt.log"
readonly REGEX_PATTERNS="^(?!.*\/?!*(\.git|vendor|CHANGELOG.md)).*\.(sh)$"
...
# Run analyzer
if [[ -n "${LIST}" ]]; then
  readonly CLI="shfmt -ln bash -sr -ci -i 2 -d"
