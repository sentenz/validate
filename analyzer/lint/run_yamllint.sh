...
readonly FILE_CONF=".yamllint.yml"
readonly FILE_LOG="${PATH_SCRIPTDIR}""/yamllint.log"
readonly REGEX_PATTERNS="^(?!.*\/?!*(\.git|vendor|CHANGELOG.md)).*\.(yml|yaml)$"
...
# Run analyzer
if [[ -n "${LIST}" ]]; then
  readonly CLI="yamllint --no-warnings"
