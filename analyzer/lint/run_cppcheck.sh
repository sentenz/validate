...
readonly FILE_CONF=".cppcheck-suppressions"
...
# Run analyzer
if [[ -n "${LIST}" ]]; then
  readonly CLI="cppcheck --enable=warning --suppressions-list=${PATH_CONFDIR}/${FILE_CONF} --template='[{file}:{line}]:({severity}),{id},{message}' --force -q"
